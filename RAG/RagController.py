...
async def index_document(file: UploadFile = File(...)):
    ...
    # Validazione dimensione file (es. max 50MB)
    MAX_FILE_SIZE = 50 * 1024 * 1024  # 50MB
    
    temp_file_path = TEMP_UPLOAD_DIR / f"temp_{file.filename}"
    
    try:
        # Salva il file temporaneamente con verifica dimensione
        file_size = 0
        with open(temp_file_path, "wb") as buffer:
            while chunk := await file.read(8192):  # Leggi in chunk da 8KB
                file_size += len(chunk)
        ...
        logger.info(f"File ricevuto: {file.filename} ({file_size / 1024:.2f} KB)")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore durante l'indicizzazione: {str(e)}"
        )
    finally:
